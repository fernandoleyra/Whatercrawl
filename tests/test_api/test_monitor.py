...
@pytest_asyncio.fixture
async def client_with_change():
    """Fixture for testing change detection — extractor returns NEW_CONTENT."""
    mock_crawler = MagicMock()
    mock_crawler.crawl_url = AsyncMock(return_value={
        "url": "https://example.com", "html": "<p>" + "word " * 30 + "</p>",
        "screenshot_b64": "", "status_code": 200, "error": None,
    ...
    mock_crawler.close = AsyncMock()
    mock_extractor = MagicMock()
    mock_extractor.extract_markdown = MagicMock(return_value=NEW_CONTENT)
    mock_job_store = MagicMock()
    mock_job_store.init = AsyncMock()
    mock_job_store.close = AsyncMock()
    mock_job_store.create_job = AsyncMock(return_value="snap-001")
...
async def test_check_detects_change(client_with_change):
    """Test that check endpoint detects content changes and populates diff."""
    response = await client_with_change.post("/monitor/check", json={"snapshot_id": "snap-001"})
    assert response.status_code == 200
    body = response.json()
    assert body["diff"]["changed"] is True
    assert len(body["diff"]["added"]) > 0
